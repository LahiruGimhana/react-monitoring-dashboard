  ...
  return (
    <div>
      <input
        ...
        onChange={() => {
          if (currentTheme === "light") {
            return changeCurrentTheme("dark");
          } else {
            return changeCurrentTheme("light");
          }
        }}
      />
      <label
        className="flex items-center justify-center cursor-pointer w-8 h-8 bg-slate-100 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600/80 rounded-full"
      ...
      >
        ...
        <svg
          className="w-8 h-8 hidden dark:block mt-3 ml-2"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            className="fill-current text-slate-400"
